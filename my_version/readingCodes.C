...
public:
  ...
  //Gabriel Botogoske modificou algumas coisas
  ...
  Double_t chargeTime_menor = 18000; // last time to integrate
  ...
  void readFile(string file){
    ...
    t2 = new TTree("t2","ADC processed waveform");
    for(Int_t i = 0; i<channels.size(); i++){
  ...
  void readFiles(string files){
    ...
    while(!filesdata.fail()){
      ...
      if(first_file){
        ...
        bch_t = t2->Branch(Form("Ch_t"),&ch_t,ch_t.tobranch.c_str());
        f1->Write();
...
//     f1->Close();
  ...
  void readData(vector<string> file_ch,string rootfile, Double_t &tEvent){
    ...
    for(Int_t i = 0; i<channels.size(); i++){
     ...
     bch_t = t2->GetBranch(Form("Ch_t"));
     bch_t->SetAddress(&ch_t);
...
//       cout << i << " " << ch.wvf[i] << endl;
      ...
      if(i>=startCharge/dtime && i<chargeTime_menor/dtime){
        ch.charge_m+=filtered[i]*dtime;
      }
