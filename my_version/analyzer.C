...
class ANALYZER{
  public:
    ...
    vector<ADC_DATA> ch_a;
    vector<ADC_DATA> ch_w;
    ...
    void setAnalyzer(string m_filename = "analyzed.root"){
      ...
      ch.resize(nchannels);
      ch_a.resize(nchannels);
      ch_w.resize(nchannels);
    ...
    void applyFreqFilter(Double_t *_filtered = nullptr){
      ...
      for(Int_t i = 0; i < memorydepth; i++){
        ...
        ch_w[kch].wvf[i]= w->hwvf->GetBinContent(i+1);
      }
       //w->hwvf->Draw("");
    }
    ...
    void drawGraph_average(string opt = "", Int_t n = memorydepth, Double_t* x = nullptr, Double_t* y = nullptr){
      if (opt == "") opt = plot_opt;
      if (x == nullptr) x = time;
      if (y == nullptr) y = ch_a[kch].wvf;
      gwvf = new TGraph(n,x,y);
      gwvf->Draw(opt.c_str());
      gwvf->GetXaxis()->SetTitle(xlabel.c_str());
      gwvf->GetYaxis()->SetTitle(ylabel.c_str());
     ...
     void drawGraph_wiener(string opt = "", Int_t n = memorydepth, Double_t* x = nullptr, Double_t* y = nullptr){
      if (opt == "") opt = plot_opt;
      if (x == nullptr) x = time;
      if (y == nullptr) y = ch_w[kch].wvf;
      gwvf = new TGraph(n,x,y);
      gwvf->Draw(opt.c_str());
      gwvf->GetXaxis()->SetTitle(xlabel.c_str());
      gwvf->GetYaxis()->SetTitle(ylabel.c_str());
    ...
    void averageWaveform(Int_t maxevent = 0, string selection = "", Double_t filter =  0){
      ...
      for(Int_t i = 0; i < nev; i++){
        ...
        for (Int_t j = 0; j < memorydepth; j++){
          ...
          ch_a[kch].wvf[j]+=ch[kch].wvf[j];
        }

      }
       for (Int_t j = 0; j < memorydepth; j++){
          ch_a[kch].wvf[j]=ch_a[kch].wvf[j]/total;
        }
